...
<main>
  ...
  <section id="form-view" class="panel" hidden>
    ...
    <form id="event-form" class="form-grid">
      ...
      <label>Physisches Modul / Seriennummer <select id="module-serial"><option value="">Nur Position / keine eindeutige Zuordnung</option></select><span class="hint">Nur nachweisbare Identitäten; Position und physisches Modul bleiben getrennt.</span></label>
...
function setSerialOptions(event){{const select=byId('module-serial');select.replaceChildren();const none=document.createElement('option');none.value='';none.textContent='Nur Position / keine eindeutige Zuordnung';select.append(none);if(event?.module_serial){{const known=document.createElement('option');known.value=event.module_serial;known.textContent='Im Eintrag gespeichert: '+event.module_serial;select.append(known);}}select.value=event?.module_serial||'';}}
