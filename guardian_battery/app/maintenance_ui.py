...
<main>
  ...
  <section class="panel">
    <div class="toolbar"><h2>Hycube History Projection</h2><button id="projection-backfill" type="button">Historische Hycube-Projektion aufbauen</button></div>
    <p class="muted">Technischer, aus Raw Evidence rekonstruierbarer Lesepfad. Kein Batterie-, Alarm- oder Cell-Risk-Status.</p>
    <dl id="projection-status" class="facts"><div><dt>Status</dt><dd>nicht verfügbar</dd></div></dl>
  </section>
...
async function loadProjection(){{const host=byId('projection-status');try{{const data=await apiFetch('api/hycube-projection/status'),live=data.live||{{}},backfill=data.backfill||{{}},source=data.history_source||{{}},rows=[['Status',live.state],['Projection Days',live.projection_days],['Complete',live.complete_days],['Open',live.open_days],['Invalid',live.invalid_days],['Storage Bytes',live.projection_bytes],['Last Projection Write',live.last_success],['Failures',live.failure_count],['Buffered Records',live.buffered_records],['Backfill Status',backfill.status],['Progress',`${{backfill.files_completed??0}} / ${{backfill.files_total??0}}`],['Current Day/File',backfill.current_file],['Records',backfill.records_written],['Bytes',backfill.bytes_processed],['Errors',backfill.errors],['Last History Source Mode',source.mode],['Raw Fallback Reason',source.fallback_reason]];host.replaceChildren();for(const [label,value] of rows){{const wrap=document.createElement('div'),dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=label;dd.textContent=value??'nicht verfügbar';wrap.append(dt,dd);host.append(wrap);}}}}catch{{host.replaceChildren();const wrap=document.createElement('div'),dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent='Status';dd.textContent='nicht verfügbar';wrap.append(dt,dd);host.append(wrap);}}}}
async function startProjectionBackfill(){{const button=byId('projection-backfill');button.disabled=true;try{{const result=await apiFetch('api/hycube-projection/backfill',{{method:'POST'}});setMessage(result.accepted?'Backfill wurde angefordert.':'Backfill ist bereits angefordert.');await loadProjection();}}catch(error){{setMessage(friendlyError(error),'error');}}finally{{button.disabled=false;}}}}
